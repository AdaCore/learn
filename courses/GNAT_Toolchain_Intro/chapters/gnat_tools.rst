...
:program:`gnatchop` renames files to match the file structure and
...
files and implementations (bodies) to be stored in :file:`.adb`
...
However, we may have multiple Ada packages contain in a single
...
To compile this code, we first call :program:`gnatchop` on the file
containing our source code before we then call :program:`gprbuild`:
...
:file:`p_main.adb` using the package names in :file:`example.ada`.
...
way to achieve this is using a source-code preprocessor. However, in
many cases where conditional compilation is needed, we can instead use
the syntax of the Ada language or the functionality provided by
...
In complex cases, using a preprocessor is ofen the most
straightforward option.  When we encounter such a case, we can use
...
:program:`gnatprep`. Also, you'll encounter some differences in the
syntax, such as shown in the example below:
...
use it with the above code snippet. This is the complete package body
stored in file :file:`do_something.org.adb`:
...
code for that subprogram, thus reducing the overall code size.  We can
use this pragma manually, but our task is simplified by using
...
(:file:`main_app.adb` in our case) and the file we want to check for
unused code. In this example:
...
in the GNAT User's Guide contains further examples and describes the
...
in the GNAT User's Guide for a more detailed discussion of
:program:`gnatmem`.
...
The simplest way to use this tool is to provide the name of the project or
invoking :program:`GNATdoc` from a directory containing a project file:
...
Using just this command is sufficient if your goal is to generate a simple
list of the packages and a list of subprograms in each. However, to create
more meaningful documentation, you can annotate the source code to add a
...
element as a comment in the line below the actual declaration. It will also
be able to extract the package description, which we wrote as a comment in
the line right after :ada:`package P is`. Finally, it will extract the
documentation of function :ada:`Add_One` (both the description of the
:ada:`V` parameter and the return value).
...
Or you can use the ``-p`` switch to request :program:`GNATdoc` to parse the
...
source-code from a project.  You can use this XML representation for
source-code analysis.  Use the tool by naming the project file when calling
it:
...
default, so we usually need to redirect it to a file (as seen in the
example above).  In that example, the :file:`output.xml` file receives an
XML representation of the complete source code of our project.

Alternatively, we can generate an XML file corresponding to each source
file from our project. To do that, we use the ``--output-dir`` option:
...
Although the source in our example is very small, the corresponding XML
representation is quite verbose because :program:`gnat2xml` classifies each
syntactical element in the source and produces a detailed description of
each.
...
many coding styles. These options are extensively discussed in the `section
on gnatpp
...
that job for you. For example, let's consider the following package
specification:
...
As we can see in this example, not only has :program:`gnatstub` put all the
elements from the package specification into the package body that it
generated, but it also created:
...
in the GNAT User's Guide for a detailed discussion of :program:`gnatstub`
and its options.
...
the our project's package specification to the AUnit framework.

In this section, we'll reuse the package specification from the previous
example.  This was the specification of the :ada:`Aux` package:
...
We'll find the complete generated code in :file:`obj/gnattest`. We can
...
If we had implemented the actual unit tests, we'd see the test results at
...
behavior looks like when the test is implemented, we can simply replace the
calls to :ada:`Assert` in that file by :ada:`null`. If we do that and
rebuild the application, we get:
